...
export default function Admin() {
  ...
  const fetchBusinessStats = async () => {
    try {
      ...
      const businessStats: BusinessStats[] = businessData?.map(business => {
        const clientCount = business.clients?.length || 0;
        const invoices = business.invoices || [];
        const invoiceCount = invoices.length;
        const totalRevenue = invoices.reduce((sum: number, invoice: any) => sum + (invoice.total_amount || 0), 0);
